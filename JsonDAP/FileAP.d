...
public static class CL_FileAP
{
    ...
    public static Optional!T readJsonFile(T)(string filePath)
    {
        try
        {
            ...
            CL_PublicCodeOtherCode.StrIsNUll(filePath, "input",
                    CheckStrIsNull, errorTextStrIsNull);
            if (CheckStrIsNull)
            {
                throw new InvalidArgumentExceptionAP("Input file path cannot be empty or null.",
                        "filePath", filePath, "Non-empty string expected", null, 0);
            }

            enforce(existsFile(filePath), throw new FileOperationExceptionAP(
                    "File not found for reading.", filePath, "read", 0, null, 0));
        ...
        {
            throw new FileOperationExceptionAP("File I/O error reading file.",
                    filePath, "read", 0, null, 0, e);
        }
        catch (JSONException e)
        {
            throw new JsonOperationExceptionAP("Invalid JSON format in file.",
                    filePath, null, null, 0, e);
        }
        catch (Exception e)
        {
            throw new UnknownErrorexceptionAP(
                    "An unexpected error occurred while reading JSON file.", null, 0, e);
        }
    }
    ...
    public static bool writeJsonFile(T)(string filePath, T data)
    {
        ...
        {
            throw new FileOperationExceptionAP("File I/O error writing to file.",
                    filePath, "write", 0, null, 0, e);
        }
        catch (JSONException e)
        {
            throw new JsonOperationExceptionAP("Serialization error while writing JSON to file.",
                    filePath, null, null, 0, e);
        }
        catch (Exception e)
        {
            throw new UnknownErrorexceptionAP(
                    "An unexpected error occurred while writing JSON file.", null, 0, e);
        }
    }
    ...
    public static Optional!long getJsonFileSize(string filePath)
    {
        try
        {
            if (filePath.empty)
            {
                throw new InvalidArgumentExceptionAP("File path cannot be empty when getting file size.",
                        "filePath", filePath, "Non-empty string expected", null, 0);
            }
            enforce(existsFile(filePath), throw new FileOperationExceptionAP(
                    "File not found for size check.", filePath, "get size", 0, null, 0));
            long fileSize = std.file.getSize(filePath);
            return Optional!long(fileSize);
        }
        catch (FileException e)
        {
            throw new FileOperationExceptionAP("File I/O error getting size of file.",
                    filePath, "get size", 0, null, 0, e);
        }
        catch (Exception e)
        {
            throw new UnknownErrorexceptionAP(
                    "An unexpected error occurred while getting file size.", null, 0, e);
        }
    }
    ...
    public static Optional!(T[]) readJsonArray(T)(string filepath)
    {
        try
        {
            ...
            enforce(existsFile(filePath)), throw new FileOperationExceptionAP(
                    "File not found for reading array.", filePath, "read array", 0, null, 0);
            string jsonContent = readText(filePath);
            return deserializeJsonArray!T[](jsonContent);
        }
        catch (FileException e)
        {
            throw new FileOperationExceptionAP("File I/O error reading array from file.",
                    filePath, "read array", 0, null, 0, e);
        }
        catch (JSONException e)
        {
            throw new JsonOperationExceptionAP("Invalid JSON format for array in file.",
                    filePath, null, null, 0, e);
        }
        catch (Exception e)
        {
            throw new UnknownErrorexceptionAP(
                    "An unexpected error occurred while reading JSON array from file.", null, 0, e);
        }
    }
    ...
    public static bool existsFile(string filePath)
    {
        ...
        {
            throw new FileOperationExceptionAP("File I/O error during existence check.",
                    filePath, "check existence", 0, null, 0, e);
        }
        catch (Exception e)
        {
            throw new UnknownErrorexceptionAP(
                    "An unexpected error occurred during file existence check.", null, 0, e);
        }
...
public static class CL_FileAP_Edit
{
    ...
    public static bool updateJsonValueOBJECT(V)(string filePath, string jsonPath, V value)
    {
        ...
        {
            throw new ArgumentNullExceptionAP(format!"filePath : %s -- JsonPath : %s"(checkStrIsNull[0],
                    checkStrIsNull[1]));
        }

        if (!CL_FileAP.existsFile(filePath))
        {
            throw new FileOperationExceptionAP("The file does not exist.", filePath, "read", 2);
        }
    ...
    public static bool updateJsonValueARRAY(V)(string filePath, string jsonPath, V value)
    {
        ...
        for (int i = 0; i < parsedSteps.length - 1; i++)
        {
            ...
            if (currentJsonNodeToTraverse.type != JSONType.OBJECT
                    && currentJsonNodeToTraverse.type != JSONType.ARRAY)
            {
                return false;
            }
        }
    ...
    public static bool addJsonItemARRAY(T)(string filePath, string jsonPath, T item)
    {
        ...
        for (int i = 0; i < parsedSteps.length - 1; i++)
        {
            ...
            if (currentJsonNodeToTraverse.type != JSONType.ARRAY
                    && currentJsonNodeToTraverse.type != JSONType.OBJECT)
            {
                return false;
            }
        }
    ...
    public static bool removeJsonItemARRAY(string filePath, string jsonPath)
    {
        ...
        for (int i = 0; i < parsedSteps.length - 1; i++)
        {
            ...
            else if (currentStep.type == PathStepType.ArrayIndex)
            {
                ...
                if (currentStep.index < 0
                        || currentStep.index >= currentJsonNodeToTraverse.array.length)
                {
                    return false;
                }
        ...
        else if (finalStep.type == PathStepType.ArrayIndex)
        {
            ...
            *currentJsonNodeToTraverse = JSONValue(currentJsonNodeToTraverse.array[0 .. finalStep.index]
    ...
    public static bool mergeJson(string filePath, JSONValue jsonContentToMerge,
            bool overwriteExisting = true, bool mergeArrays = false)
    {
        string errorCheck;
        ...
        if (outputBoolCheck)
        {
            throw new InvalidArgumentExceptionAP("FilePath is Emtpy, error : " ~ errorCheck,
                    "||Empty||", "Null or", "Empty");
        }
        ...
        string mergeMessage = "";
        bool mergeResult = CL_JsonOtherCode.recursiveMerge(rootJson,
                jsonContentToMerge, overwriteExisting, mergeArrays, mergeMessage);
        if (!mergeResult)
        {
            throw new BaseExceptionAP(format!"can't Merge - Error Message = %s"(mergeMessage),
                    __FILE__, __LINE__);
        }

        return CL_FileAP.writeJsonFile(filePath, rootJson);
...
public class CL_File_JSON
{
    ...
    public static JSONValue[] serializeToJsonArray(T)(T data)
    {
        ...
        {
            throw new JsonOperationExceptionAP("Failed to serialize object to JSON array string.",
                    null, data.toString(), null, 0, e);
        }
    }
